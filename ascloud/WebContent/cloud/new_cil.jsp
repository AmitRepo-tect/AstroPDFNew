...
<%@page import="com.ojassoft.astrosagecloud.DO.BeanKpCIL"%>
...
<div class="hdg-lg card">
	<h1><%=rbConstants.getString("CUSPAL_INTERLINK_SUB") %></h1>	
</div>

<%
	
	InPlanetKpCusp inPlanetCusp = (InPlanetKpCusp) session.getAttribute("KPObj");
	BeanKpCIL[] beanKpCIL = KCILUtil.getKPCILBeansArray(inPlanetCusp);
     String[] arr = new String[]{"PLANET","STAR_LORD_OCCUPANCY","OCCUPANCY","STAR_LORD_OWNERSHIP","OWNERSHIP"};
...
<thead>
	<tr class="amber lighten-5">
       ...
       <th class="cellhead">
         ...
         <!--1-->
         <% for(int tcount=0;tcount<12;tcount++) { %>
       <tr>
        <th class="cellhead">
                <%=tcount+1%>
         </th>
        ...
        <td class="cellcontent">
          <%=KCILUtil.getFormattedLevelValues(beanKpCIL[tcount].getKpCILType1(), true)%>
        </td>
        <td class="cellcontent">
          <%=KCILUtil.getFormattedLevelValues(beanKpCIL[tcount].getKpCILType2(), true)%>
        </td>
        <td class="cellcontent">
          <%=KCILUtil.getFormattedLevelValues(beanKpCIL[tcount].getKpCILType3(), true)%>
        </td>
        <td class="cellcontent">
          <%=beanKpCIL[tcount].getKpCILType4()%>
        </td>
           </tr>
        <% } %>
	</table>
...
<div class="col s12 l12">
<div class="card table-panel">
<table class="highlight striped bordered ui-table">
<% for(int icount=0;icount<=3;icount++) { %>
        ...
        <td class="cellcontent">
          <b><%=rbConstants.getString(contentarray[icount])%></b>
        </td>
...
<center> 
 <canvas id="CanvasKpPCusp" width="600px" height="400px" align="center"
					class="ui-canvas mrt-20"></canvas></center>
