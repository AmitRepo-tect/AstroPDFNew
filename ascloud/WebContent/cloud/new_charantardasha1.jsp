...
<%         
          ...
          int count = 0;
          int asplittedCells = 0;
          int asplittedLines = 0;
          // int Count=0 ;
           ...
           // splittedCells = Split(DashaOutput, "+"&vbLf) 
        		   
          // String[] splittedCells = DashaOutput.split(""+(char)43);
           String[] splittedCells = DashaOutput.split("\\+"+(char)10);
          // out.println(DashaOutput+"<br/><br/>");
        //   out.println(String.valueOf((char)43)+(char)10+""+"<br/><br/>");
          // out.println(splittedCells[0]);
          // out.println(splittedCells[1]+"<br/>");
          // out.println(splittedCells[2]+"<br/>");
          // out.println(splittedCells[3]);
           %>
<div class="mrt-30"> 
<div class="card"> 
	...
	<% for(int i = 0 ;i<= 3 ; i++){
     %>
		...
		<div class="col s6 altcolor-oe">
            ...
            //Split(splittedCells(i), vbLf)%> <%for(int j = 0 ;j<splittedLines.length ;j++){
            	 // out.println(splittedLines.length);%>
			<table width="100%" class="vim-tablenn cr-font">
				<% if(count==0){ %>
				
				<% count=count+1 ;
                } %>
				<tr>
					<td> <% if((String.valueOf(session.getAttribute("languageCode")).equals("0"))|| (String.valueOf(session.getAttribute("languageCode")).equals("1")))
                            {
...
<%       
   ...
   <b> <%=rBConstants.getString("NOTE") %> : <%=rBConstants.getString("DATES_GIVEN_ABOVE_ARE_DASHA_ENDING_DATES") %>
			...
			<a href="charantardasha2.asp" class="waves-effect waves-light btn amber darken-4 box-shadow"> <%=rBConstants.getString("NEX") %></a>
			</div>
   </div>
   </div>
