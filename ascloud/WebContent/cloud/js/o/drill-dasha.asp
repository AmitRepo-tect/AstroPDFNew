<% Response.Charset = "utf-8" %>DegreeofMoon = <%=request("moon")%>;DayofBirth = <%=Cint(request("DoB"))%>;MonthofBirth = <%=Cint(request("MoB"))%>;YearofBirth = <%=Cint(request("YoB"))%>;HourofBirth = <%=Cint(request("Hour"))%>;MinuteofBirth = <%=Cint(request("Min"))%>;SecondofBirth = <%=Cint(request("Sec"))%>;LanguageCode = <%=Cint(request("LanguageCode"))%>;
var total = 120;
...
planetNumber['Me'] = 8;
 ...
 $("div.dasa").append('<div id="PlanetaryTableTD" class="row"><div id="PlanetaryTableTD" class="planet">' + levelName[level] + seperator + planets[i] + '</div><div id="PlanetaryTableTD" class="date">' + dasa(i) + '</div></div>');
 } 
 installClickHandler();
 }
 function dasa(i) {
 ...
 if(LanguageCode==5){
     ...
     if(levelName[level].indexOf("-")>0){arStartPlanet[level] = planetNumber[planetval[planetval.length-1].trim()]}
     else{arStartPlanet[level] = planetNumber[Right(levelName[level], planetWidth)];}
 }
 else{    
