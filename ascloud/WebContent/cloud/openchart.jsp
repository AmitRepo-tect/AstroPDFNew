...
<script type="text/javascript">
    ...
    function openChart(val) {
        ...
        $("#chartname").val();
        srno = 1;
        $.ajax({url: "sessioncheckofuserid.jsp", success: function(result){
                if(result.trim() == null   || result.trim() == "" || result.trim() == "null"  )
            	{
            	if (val == 'open')
                    v = 'loginwelcome.asp?val=open';
                if (val == 'manage')
                    v = 'loginwelcome.asp?val=manage';
            ...
            if (confirmation) {
                var value = (function() {
                    ...
                    $.ajax({
                        'async': false,
                        'global': false,
                        'url': 'ChartServlet?methodName=deleteChart',
    ...
    function CreatedataList(li) {
        if (type == "open") {
            window.location = "ChartServlet?methodName=openChart&ChartId=" + li.extra[0];
        }
        var strChartData = "";
        var strTableHeading = "";
        ...
        if (type == 'open') {
            strChartData = strChartData + '<td><a href="ChartServlet?methodName=openChart&ChartId=' + li.extra[0] + '" class="pseudo"><%=openChartConstants.getString("OPENN") %></a></td></tr>';
        }
        if (type == "manage") {

            chartIdAndChartName = "'" + li.extra[0] + "," + $("#chartname").val() + "'";
            strChartData = strChartData + '<td><a href="ChartServlet?methodName=openChart&ChartId=' + li.extra[0] + '" class="pseudo"><%=openChartConstants.getString("OPENN") %></a></td>';
...
*/
    ...
    function loadMore() {
        ...
        $.ajax({
            url: 'ChartServlet?methodName=loadCharts',
            method: 'post',
            ...
            success: function(data) {
                ...
                if (val != "" && val != "You are not authorized to view this page") {
                    ...
                    for (var i = 0; i < chartData.length; i++) {
                        ...
                        if (type == 'open') {
                            strChartData = strChartData + '<td><a href="ChartServlet?methodName=openChart&ChartId=' + chartData[i].chartId + '" class="pseudo"><%=openChartConstants.getString("OPENN") %></a></td></tr>';
                        }
                        if (type == "manage") {

                            chartIdAndChartName = "'" + chartData[i].chartId + "," + chartData[i].chartName + "'";
                            strChartData = strChartData + '<td><a href="ChartServlet?methodName=openChart&ChartId=' + chartData[i].chartId + '" class="pseudo"><%=openChartConstants.getString("OPENN") %></a></td>';
