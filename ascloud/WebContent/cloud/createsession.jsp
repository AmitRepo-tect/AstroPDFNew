...
<%
	if (place.equals("")) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Please specify place of birth.", "UTF-8"));
		}

		if (String.valueOf(longDegree).equals("")) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Please specify a value for the Longitude Degree.", "UTF-8"));
		}

		if (!(IsNumeric(String.valueOf(longDegree)))) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Incorrect value for the Longitude Degree.", "UTF-8"));
		}
		...
		if (String.valueOf(longMinute).equals("")) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Please specify a value for the Longitude Minute.", "UTF-8"));
		}
		...
		if (longMinute < 0 || longMinute >= 60) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Incorrect value for the Longitude Minute.", "UTF-8"));
		}

		if (String.valueOf(latDegree).equals("")) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Please specify a value for the Latitude Degree.", "UTF-8"));
		}

		if (!(IsNumeric(String.valueOf(latDegree)))) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Incorrect value for the Latitude Degree.", "UTF-8"));
		}
		...
		if (String.valueOf(latMinute).equals("")) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Please specify a value for the Latitude Minute.", "UTF-8"));
		}

		if (!(IsNumeric(String.valueOf(latMinute)))) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Incorrect value for the Latitude Minute.", "UTF-8"));
		}
		...
		if (String.valueOf(timeZone).equals("")) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Please specify a value for the Time Zone.", "UTF-8"));
		}

		if (!(IsNumeric(String.valueOf(timeZone)))) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Incorrect value for the Time Zone.", "UTF-8"));
		}
		...
		if (!(IsNumeric(String.valueOf(languageCode)))) {
			proceed = false;
			response.sendRedirect("http://www.astrosage.com/m/postpage.asp?type=kp&error="
					+ URLEncoder.encode("ERROR: Incorrect value for the Language Code.", "UTF-8"));
		}
...
<%
	if (proceed) {
			...
			if (!Util.safestr(request.getParameter("doneandsave")).equals("")
					&& session.getAttribute("User_Id") != null && !session.getAttribute("User_Id").equals("")) {
				session.setAttribute("IsSaved", 0);
				// Save Chart functionality 
				UserDAO dao = new UserDAO();
				HashMap<String, Integer> hm = dao.saveChart(ObjHoro, session.getAttribute("User_Id").toString(),
						request.getParameter("charting"), "", "Web", "", 0, null, false);
