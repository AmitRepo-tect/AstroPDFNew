...
<%@page import="com.ojassoft.astrosagecloud.DO.BeanKpPlanetSigWithStrength"%>
...
<div class="hdg-lg card">
	<h1><%=rbConstants.getString("PLANET_SIGNVIEW2") %></h1>	
</div>

<%
	
	InPlanetKpCusp inPlanetCusp = (InPlanetKpCusp) session.getAttribute("KPObj");
	BeanKpPlanetSigWithStrength[] beanPlanetSign = KCILUtil.getPlanetSignWithStrengthBeansArray(inPlanetCusp);
...
<tr class="amber lighten-5">
       ...
       <th class="cellhead">
         ...
         <% for(int tcount=0;tcount<9;tcount++) { 
        	 String str = "";
        	 int planetStren = beanPlanetSign[tcount].getPlanetStrength();
        	 if (planetStren == 1)
        		 str = "+";
         %>
       <tr>
        <th class="cellhead">
                <%=rbConstants.getString(Common.arrShortPlanetName[beanPlanetSign[tcount].getPlanet()].toUpperCase())+str%>
         </th>
     
        <td class="cellcontent">
          <%=beanPlanetSign[tcount].getLevel1()%>
        </td>
        <td class="cellcontent">
          <%=beanPlanetSign[tcount].getLevel2()%>
        </td>
        <td class="cellcontent">
          <%=KCILUtil.getFormattedLevelValues(beanPlanetSign[tcount].getLevel3(), false)%>
        </td>
        <td class="cellcontent">
          <%=KCILUtil.getFormattedLevelValues(beanPlanetSign[tcount].getLevel4(), false)%>
        </td>
           </tr>
       <% } %>
</table>
