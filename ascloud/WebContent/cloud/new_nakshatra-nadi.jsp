...
<%@page import="com.ojassoft.astrosagecloud.DO.BeanNakshtraNadi"%>
...
<div class="hdg-lg card">
	<h1><%=rbConstants.getString("NAKSHATRA_NADI") %></h1>	
</div>

<%
	
	InPlanetKpCusp inPlanetCusp = (InPlanetKpCusp) session.getAttribute("KPObj");
	BeanNakshtraNadi[] beanNakNad = KCILUtil.getNakshtraNadi(inPlanetCusp);
	ArrayList<String[][]> arr2 = initView(beanNakNad, rbConstants);
     String[] arr = new String[]{"PLANET","STAR_LORD","SUB_LORD"};
...
<tr class="amber lighten-5">
       ...
       <tr>
        <td>
             <%=getFormattedLevelValues(arr2, tcount, 0, languageCode) %>
         </td>
     
        <td>
          <%=getFormattedLevelValues(arr2, tcount, 1, languageCode)%>
        </td>
        <td>
          <%=getFormattedLevelValues(arr2, tcount, 2, languageCode)%>
        </td>
...
private ArrayList<String[][]> initView(BeanNakshtraNadi[] nakBeans, MyProperties rbConst) {
    ...
    for (int i = 0; i < nakBeans.length; i++) {
        ...
        arr2[0][0] = rbConst.getString(Common.arrLongPlanetName[nakBeans[i].getPlanetNakstraNadi().getPlanet()].toUpperCase());
        int[] pl = nakBeans[i].getPlanetNakstraNadi().getNadi();
        //ADDED BY BIJENDRA ON 19-MARCH-2013
        if (pl != null)
            pl = KCILUtil.removeUpdatedDuplicates(pl);
        //END
        ...
        arr2[1][0] = rbConst.getString(Common.arrLongPlanetName[nakBeans[i].getPlanetStarLordNadi().getPlanet()].toUpperCase());
        int[] pll = nakBeans[i].getPlanetStarLordNadi().getNadi();
        //ADDED BY BIJENDRA ON 19-MARCH-2013
        if (pll != null)
            pll = KCILUtil.removeUpdatedDuplicates(pll);
        ...
        arr2[2][0] = rbConst.getString(Common.arrLongPlanetName[nakBeans[i].getPlanetSubLordNadi().getPlanet()].toUpperCase());
        int[] plll = nakBeans[i].getPlanetSubLordNadi().getNadi();
        //ADDED BY BIJENDRA ON 19-MARCH-2013
        if (plll != null)
            plll = KCILUtil.removeUpdatedDuplicates(plll);
        //END
...
private String getFormattedLevelValues(ArrayList<String[][]> arr, int beanIndex, int rowType, int languageCode) {
    String value = "";
